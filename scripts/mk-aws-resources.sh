...
for env in production; do
    $aws ec2 create-security-group --group-name "birdman-peer-$env" --description "Birdman - peer $env" || true
    $aws dynamodb create-table --table-name birdman_$env \
